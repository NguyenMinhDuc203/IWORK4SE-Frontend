...
export function ApplicantContactButton({
  ...
  const handleOpenChange = async (open: boolean) => {
    ...
    if (open && !applicantEmail && onFetchEmail) {
      setIsLoadingEmail(true)
      try {
        const email = await onFetchEmail()
        if (email) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogTrigger asChild>
        <Button size={triggerSize} variant={triggerVariant} className={className} onClick={() => setIsOpen(true)}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Chọn cách bạn muốn liên hệ với {applicantName}</DialogDescription>
        </DialogHeader>
          ...
          <div className="grid gap-4 py-4">
            ...
            >
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-muted-foreground break-all">{applicantEmail || "Đang tải email..."}</p>
              </div>
            </button>
          </div>
        )}
